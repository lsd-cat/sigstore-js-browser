...
describe('X509Certificate', () => {
  ...
  describe('#verify', () => {
    ...
    describe('when the issuer is provided', () => {
      describe('when the issuer is a parent certificate', async () => {
        it('returns true', async () => {
          const res1 = await leafCert.verify(intCert);
          expect(res1).toBe(true);
          //expect(async () => await leafCert.verify(intCert)).toBe(true);
          const res2 = await intCert.verify(rootCert);
          expect(res2).toBe(true);
          //expect(async () => await intCert.verify(rootCert)).toBe(true);
        });
      });

      describe('when the issuer is NOT a parent', async () => {
        it('returns false', async () => {
          const res1 = await leafCert.verify(rootCert);
          expect(res1).toBe(false);
          //expect(async () => await leafCert.verify(rootCert)).toBe(false);
          const res2 = await intCert.verify(leafCert);
          expect(res2).toBe(false);
          //expect(async () => await intCert.verify(leafCert)).toBe(false);
          const res3 = await rootCert.verify(leafCert);
          expect(res3).toBe(false);
          //expect(async () => await rootCert.verify(leafCert)).toBe(false);
          const res4 = await rootCert.verify(intCert)
          expect(res4).toBe(false);
          //expect(async () => await rootCert.verify(intCert)).toBe(false);
        });
      });
    });

    describe('when the issuer is NOT provided', () => {
      describe('when the certificate is self-signed', async () => {
        it('returns true', async () => {
          const res1 = await rootCert.verify();
          expect(res1).toBe(true);
          //expect(async () => await rootCert.verify()).toBe(true);
        });
      });

      describe('when the certificate is NOT self-signed', async () => {
        it('returns false', async () => {
          const res1 = await intCert.verify();
          expect(res1).toBe(false);
          //expect(async () => await intCert.verify()).toBe(false);
          const res2 = await leafCert.verify();
          expect(res2).toBe(false);
          //expect(async () => await leafCert.verify()).toBe(false);
        });
      });
    });
  });
