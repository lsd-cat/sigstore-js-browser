...
describe('RFC3161Timestamp', () => {
  ...
  describe('verify', () => {
    describe('when the timestamp is valid', () => {
      const subject = RFC3161Timestamp.parse(ts);
      let key: CryptoKey;
      createPublicKey(publicKey).then(result => {
        key = result;
      });
      const data = artifact;
    ...
    describe('when the artifact does NOT match the one which was signed', () => {
      ...
      it('throws an error', () => {
        expect(async () => await subject.verify(data, key)).rejects.toThrow(
    ...
    describe('when the key does NOT match the signature', () => {
      ...
      let key: CryptoKey;
      createPublicKey(publicKey2).then(result => {
        key = result;
      });
    ...
    describe('when the content type is NOT signedData', () => {
      ...
      let key: CryptoKey;
      createPublicKey(publicKey3).then(result => {
        key = result;
      });
    ...
    describe('when the encapsulated content type is NOT tstInfo', () => {
      ...
      let key: CryptoKey;
      createPublicKey(publicKey4).then(result => {
        key = result;
      });
    ...
    describe('when the signed message does NOT match the tstInfo', () => {
      ...
      const publicKey5 ='-----BEGIN PUBLIC KEY-----\n' +
      ...
      let key: CryptoKey;
      createPublicKey(publicKey5).then(result => {
        key = result;
      });
